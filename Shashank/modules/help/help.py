...
from Shashank import app, CMD_HELP
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "❁ ＷＡＳＴＥ Ｘ ＵＢＯＴ - ＨＥＬＰ ＭＥＮＵ ❁"
            ac.align = "l"
...
@Client.on_message(filters.command(["plugins", "modules"], ".") & filters.me | filters.user(SUDO_USER))
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "Stranger-UserBot Plugins"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
